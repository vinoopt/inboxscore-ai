...
if SENTRY_DSN:
    ...
    _version = os.environ.get("APP_VERSION", "1.16.14")
...
app = FastAPI(title="InboxScore API", version="1.16.14")
...
async def api_postmaster_compliance_bulk(req: Request):
    """INBOX-226: bulk Postmaster compliance fetch for the Dashboard.

    Originally (INBOX-211) this endpoint fan-out to Google's compliance
    API server-side, once per domain. With 5+ domains that cost 2-5s of
    every Dashboard cold load. INBOX-226 moved compliance fetching into
    the daily Postmaster scheduler (postmaster_scheduler.py) — verdicts
    are now cached in the postmaster_compliance table. This endpoint
    became a single indexed DB read.

    Response shape (unchanged from INBOX-211 — frontend reads identical
    structure):
        {
          "connected": true,
          "compliance": {
    ...
    Freshness: verdicts are at most ~24h stale (daily scheduler at
    06:00 UTC). Google's compliance verdicts are rolling/multi-day so
    this is well within useful freshness.

    First-sync edge case: on a freshly-connected user the scheduler
    hasn't run yet, so the table is empty and this returns an empty
    compliance map. The Dashboard renders the "still evaluating" banner
    per Option C — same outcome the user got before with live fetches
    against a domain Google hadn't issued verdicts for. The next 06:00
    UTC scheduler run populates the cache.
    """
    auth_header = req.headers.get("authorization", "")
    if not auth_header.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Missing authorization")
    ...
    from db import get_postmaster_compliance_bulk
    cached = get_postmaster_compliance_bulk(user_id)

    # Safety fallback (INBOX-226): if the cache is empty (migration 014
    # not yet applied, freshly-connected user before first scheduler
    # run, or row purged), fall back to the original live fan-out so
    # the Dashboard doesn't regress to an empty "still evaluating"
    # banner. The fan-out is the same code path INBOX-211 shipped —
    # slower (2-5s) but correct. Once the scheduler populates the
    # cache, the fan-out never runs again.
    if cached:
        return {"connected": True, "compliance": cached}

    import asyncio
    from postmaster import (
        ensure_valid_token, get_compliance_status, get_postmaster_domains,
        _summarise_compliance,
    )
    from db import upsert_postmaster_compliance

    try:
        access_token = await ensure_valid_token(user_id, connection)
        domain_resources = await get_postmaster_domains(access_token)
    except Exception as e:
        print(f"[Postmaster] compliance-bulk fallback: token/domains error: {e}")
    ...
    async def _fetch_one(domain: str):
        async with sem:
            try:
                comp = await get_compliance_status(access_token, domain)
                # Write through to cache so subsequent loads are fast.
                summary = _summarise_compliance(comp)
                ...
                summary["has_any_verdict"] = any(
                    v is not None for v in
                    (summary["auth"], summary["spam"], summary["encryption"])
                )
                return domain, summary
            except Exception as exc:
                print(f"[Postmaster] compliance-bulk fallback: {domain} failed: {exc}")
                return domain, {"auth": None, "spam": None,
                                "encryption": None, "has_any_verdict": False}

    raw = await asyncio.gather(*[_fetch_one(d) for d in domain_names])
    summary_map = {d: s for d, s in raw}
    return {"connected": True, "compliance": summary_map}
...
async def serve_postmaster():
    # INBOX-183 Phase A1: split from email-health.html. /microsoft +
    # /blacklist still serve email-health.html until Phases B1 + C1.
    return _html("postmaster.html")
